action_counts["SELL"] += 1
        ...
        logger.info(f"📊 Decision for {symbol}: {final_decision} (BUY: {action_counts['BUY']}, SELL: {action_counts['SELL']}, HOLD: {action_counts['HOLD']})")
        
        if final_decision in ["BUY", "SELL"]:
            candles = self.fetch_live_market_data(symbol, lookback=2)
            if candles:
                last_candle = candles[-1]
                # ডিকশনারি ও অবজেক্ট উভয় ফরম্যাট নিরাপদে হ্যান্ডেল করার লজিক
                current_price = float(last_candle['close'] if isinstance(last_candle, dict) else getattr(last_candle, 'close', 0.0))
                
                # আপনার অনুরোধ অনুযায়ী বিটকয়েন এবং গোল্ড উভয়ের স্টপ লস ২ ডলার করা হলো
                if "BTC" in symbol:
                ...
                if risk_result and risk_result.get("status") == "Success":
                    ...
                    # লাইভ ট্রেড এক্সিকিউট করার মূল কমান্ড
